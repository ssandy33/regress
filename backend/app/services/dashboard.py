...
def _build_account_summary(
    ...
    """Compose the broker-reconciliation strip (#420 — R1/R2/R4-account).

    ``account_value = equity MV + option MV + cash``; ``reconciles`` compares it
    to the broker's reported net-liquidation within tolerance. Returns the
    "Connect Schwab" empty state (all null) when no broker balance is available.
    Day-change fields stay null / ``no_prior_close`` — the R3 slice fills them.
    """
...
def build_dashboard_payload(db: DBSession, today: date | None = None) -> dict:
    ...
    # Account-summary strip (#420) — broker cash + net-liq reconciliation.
    # Uses the token-row state (``schwab_configured``) rather than the
    # per-load feed flag: the account balances come from a separate
    # ``get_accounts()`` cache, independent of the quote fan-out outcome.
