...
### Frontend types — generated from the snapshot (Contract v1 Wave 3)

`frontend/api/generated/types.ts` is the **machine-generated TS-types view** of `backend/openapi/openapi.json`. It is committed to git, regenerated by an `openapi-typescript` script, and CI-checked for drift on every push.

**Regen commands:**
- Backend snapshot: `cd backend && python -m openapi.regen`
- Frontend types: `cd frontend && npm run types:regen`
- Frontend drift check (CI mirror): `cd frontend && npm run types:check`

Whenever you change a router's response shape — add a field, change a type, remove an endpoint — run BOTH regen commands in the same PR. CI's `openapi_drift_check` (backend-unit job) AND `frontend_types_drift_check` (frontend-checks job) will fail if you skip either.

**Alias-layer pattern (per PRD #262 R-3):**

`openapi-typescript` generates deeply nested types like `paths['/api/x']['get']['responses']['200']['content']['application/json']`. **That form does NOT bleed into call sites.** Every consumer references a friendly named alias defined in `frontend/api/types.js`.

To add a new alias when you add an endpoint:
1. Land the backend change + run `cd backend && python -m openapi.regen`.
2. Run `cd frontend && npm run types:regen` to refresh `frontend/api/generated/types.ts`.
3. Add a `@typedef` block to `frontend/api/types.js`:
   ```javascript
   /** @typedef {import('./generated/types').paths['/api/your/endpoint']['get']['responses']['200']['content']['application/json']} YourEndpointResponse */
   ```
4. Reference the friendly name (`YourEndpointResponse`) in the JSDoc on your `frontend/api/client.js` function — never the raw `paths[...]` form.

**`@ts-check` is NOT enabled** on `frontend/api/*.js` in Wave 3. JSDoc typedefs give IDE autocomplete but no compile-time enforcement. If you want compile-time errors on a migration, add `// @ts-check` to the top of the file you're working on — this is a per-file opt-in, not a project-wide policy.

**Per-family migration policy:**

- The PRD #262 R-5 deliverable was the **pattern**, not 100% migration coverage. Wave 3 migrated `positions` and `regression`. The other 9 router families stay hand-typed until they're touched by Platform v2 or a future feature — at which point the touching PR also migrates that family. Do not file standalone "migrate family X" issues.
- **Wave 2.5 (#311–#315) families** (`settings`, `health`, partial `assets`/`options`) — these endpoints carry `untyped` shapes (no `response_model=`) in the current snapshot. Migrating them gives you `unknown` types with no IDE help. Wait for #311–#315 to close before adding aliases for these families.
