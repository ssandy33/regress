...
/**
 * AccountSummaryRow — broker-reconciliation strip (issue #420, PRD #415 R1/R2/R3).
 *
 ...
 *     with the breakdown as subtext and a `data-reconciles` flag.
 *   - Cash & sweep — sourced from the broker balance, never inferred (R2).
 *   - Day change — account-level $ / % (R3; renders "no prior close" until the
 *     day-change slice lands).
 *
 * States (design spec §States):
 *   - loading → 3 skeleton tiles.
 *   - unavailable (broker not connected → `summary.cash == null`) → tiles show
 *     `—` with a "Connect Schwab to reconcile" hint. Distinct from a false `$0`.
...
export default function AccountSummaryRow({ summary, loading }) {
  ...
  const dayPopulated = connected && dayState === 'populated' && summary.day_change != null;
