...
|--------------|---------------|
| See if an asset is trending up or down | **Linear** |
| Test what drives an asset's price | **Multi-Factor** |
| Find when trends started or broke | **Rolling** |
| Compare performance across assets | **Compare** |
`;

const components = {
  h1: ({ children }) => (
    <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-6 pb-3 border-b border-slate-200 dark:border-slate-700">
      {children}
    </h1>
  ),
  h2: ({ children }) => (
    <h2 className="text-2xl font-semibold text-slate-900 dark:text-white mt-10 mb-4 pb-2 border-b border-slate-200 dark:border-slate-700">
      {children}
    </h2>
  ),
  h3: ({ children }) => (
  ...
  h4: ({ children }) => (
    <h4 className="text-base font-semibold text-slate-700 dark:text-slate-300 mt-4 mb-2">
      {children}
    </h4>
  ),
  p: ({ children }) => (
  ...
  ul: ({ children }) => (
    <ul className="list-disc list-inside text-sm text-slate-700 dark:text-slate-300 space-y-1 mb-4 ml-2">
      {children}
    </ul>
  ),
  li: ({ children }) => (
  ...
  th: ({ children }) => (
    <th className="text-left px-4 py-2.5 text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider">
      {children}
    </th>
  ),
  td: ({ children }) => (
  ...
  return (
    <div className={`min-h-screen ${dark ? 'dark bg-slate-900' : 'bg-slate-50'}`}>
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <article className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-8 shadow-sm">
